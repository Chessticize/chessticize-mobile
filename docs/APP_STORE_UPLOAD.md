...
This runbook covers the owner-executed upload step for the 1.3.3 App Store
...
`altool`, or Transporter. This repository standardizes the 1.3.3 path on
...
For the 1.3.3 release, perform native build, capture, signing, and upload work
...
For 1.3.3, `config/app-store-metadata-en-us-v1.json` records the release
candidate's exact `currentVersionWhatsNew` copy. It must match
`docs/releases/ios-v1.3.3-build-1.md` before owner approval and tagging. Retain
...
The 1.3.3 candidate is explicitly **Full native validation**. Its accumulated
delta spans multiple user journeys and includes shared Detox infrastructure
changes. The bundled Core Pack remains unchanged from 1.3.2. On the accepted
...
Connect upload. The proposed iOS 1.3.3 build-1 tag is:

```sh
git tag -a ios-v1.3.3-build-1 -m "iOS 1.3.3 build 1"
git push origin ios-v1.3.3-build-1
```
...
2. Confirm the uploaded build number is `1` for version `1.3.3`.
