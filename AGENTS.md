...
- GitHub Actions does not run Xcode builds or iOS Detox. Local iOS native validation is the only iOS native release gate. Record the tested commit SHA and Git tree, build result, required suite results, Xcode version, simulator, and clean-worktree confirmation.
- Before a release, require exact-head fast checks plus the release scope selected below. An ordinary delta may ship after the owner passes the physical-device smoke without rerunning full Detox. Run the affected suite for targeted native risk and both suites only for broad native risk. A squash-merged release candidate may reuse passing PR-head native evidence only when `git rev-parse <tested-sha>^{tree}` exactly matches `git rev-parse main^{tree}`; record both tree IDs. Any tree difference or unresolved local failure that touches the changed boundary is a release blocker.
