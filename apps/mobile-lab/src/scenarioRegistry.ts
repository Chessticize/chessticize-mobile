...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "practice-custom-setup": defineScenario(
    ...
    "New Run",
    "practice--custom-setup",
    "New named run editor that defaults to a compact All themes summary, reveals the grouped server-curated 24-theme catalog on demand, matches the web timing range, lets linked warning and timeout defaults be adjusted, and reveals the default-on Opponent reply setting when Arrow Duel is selected before saving to Home without starting.",
    "practice",
    ["Required unique name", "Custom configuration", "Web timing range", "Linked timing defaults", "Independent warning and timeout controls", "Collapsed All themes summary", "On-demand 24-theme catalog", "Deterministic All-to-multiple selection", "All exclusivity", "Arrow Duel Opponent reply", "Ten-second reply default", "Thirty-second reply maximum", "Direct 600-2200 starting rating", "Add to Home"],
  ...
  "practice-arrow-duel-prompt": defineScenario("practice-arrow-duel-prompt", "Practice", "Arrow Duel choice and reply", "practice--arrow-duel-prompt", "The existing Arrow Duel session becomes a two-stage choice-and-reply challenge. A correct arrow receives brief green feedback, then animates back silently beneath a 1.5-second full-board What if preparation overlay that states the Run's actual reply time. The fixed If prompt changes before the tempting move animates. The board stays in the original solver perspective, and the separate configurable reply clock starts only when the reply position is ready for input. Terminal answers use Standard's brief board feedback and advance automatically without result copy, while every failure counts the whole puzzle as one mistake and sends it to Review.", "practice", ["Initial two-arrow choice", "Green confirmation before handoff", "Slow silent undo before tempting move", "1.5-second full-board What if overlay with configured seconds", "Fixed-height If prompt transition", "Original solver perspective retained", "Reply timer starts after animation", "Ten-second default reply", "Thirty-second maximum", "Sprint and puzzle clocks paused", "Main-line reply", "Any immediate mate accepted", "Standard-style green and red feedback", "Automatic next puzzle", "Brief timeout overlay", "Review consequence without result copy"], ["Production submission", "Review scheduling", "Runtime engine"], "contained"),
  "practice-arrow-duel-mate-in-one": defineScenario("practice-arrow-duel-mate-in-one", "Practice", "Arrow Duel reply · alternate mate", "practice--arrow-duel-reply-alternate-mate", "A sampled Core Pack position where the stored reply and another legal move both checkmate immediately. The alternate mate is accepted without consulting an engine and uses the same brief green handoff as any other correct Sprint answer.", "practice", ["Correct initial choice", "Tempting move handoff", "Stored main-line reply", "Alternate legal mate in one", "Brief green feedback", "Automatic Sprint handoff", "No runtime engine"], ["Production submission", "Review scheduling"], "contained"),
  ...
  "review-arrow-duel-reply": defineScenario("review-arrow-duel-reply", "Review", "Arrow Duel reply", "review--arrow-duel-reply", "Scheduled Arrow Duel Review inherits the Run's enabled Opponent reply rule and twelve-second fixture setting. After the fixed 1.5-second What if handoff becomes playable, Review starts that same countdown; either stage must pass for the Review to pass.", "review", ["Existing Review session", "Stored candidate order", "1.5-second What if handoff", "Run-configured twelve-second countdown", "Countdown starts at input unlock", "One Review result for both stages"], ["Review queue", "Replay"], "contained"),
  ...
  "history-filters": defineScenario("history-filters", "History", "Filters and active filters", "history--filters-and-active-filters", "The existing advanced-filter region adds Incomplete beside Correct and Wrong and as an independent OR-based Attention reason alongside Unclear and In review.", "history", ["Framed advanced filters", "Incomplete Result filter", "Incomplete Attention reason", "Attention reason OR group", "Needs attention interaction", "Sprint source default", "Selected theme names above", "Compact theme count below", "Collapsed 24-theme catalog", "Reset"], ["Production history query", "Replay puzzle"]),
  ...
  "history-arrow-duel-replay": defineScenario("history-arrow-duel-replay", "History", "Arrow Duel Replay · full line", "history--arrow-duel-replay-full-line", "History Replay inherits the enabled Opponent reply rule without a countdown. A correct candidate and reply open the remaining stored line, and Replay does not show Solved until every expected move and automatic opponent reply has been played.", "history", ["Existing History Replay", "Stored candidate order", "Untimed opponent reply", "Guided remaining line", "Solved only after the complete line", "No History or Review mutation"], ["History", "Analysis"], "contained"),
