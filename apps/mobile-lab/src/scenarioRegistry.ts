...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "practice-run-arrow-duel-editor": defineScenario("practice-run-arrow-duel-editor", "Practice", "Arrow Duel Run editor", "practice--arrow-duel-reply-setting", "Arrow Duel's existing Edit Run screen with the default-on Opponent reply control and a direct 1–60 second reply-time input. The design explains that the clock defaults to five seconds, consumes neither Sprint nor puzzle time, and keeps On and Off ratings separate without changing stored Run configuration.", "practice", ["Existing Edit Run flow", "Opponent reply default on", "Off state", "Direct 1–60 second reply-time input", "Five-second default", "No Sprint or puzzle time consumed", "Separate On and Off ratings"], ["Production persistence", "Rating-key migration", "Active Arrow Duel"]),
