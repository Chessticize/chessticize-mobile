...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "practice-tactical-profile-balanced": defineScenario("practice-tactical-profile-balanced", "Practice", "Tactical profile · no meaningful weakness", "practice--tactical-profile-balanced", "Practice Home when calibrated evidence does not support a meaningful training focus.", "practice", ["Existing Practice Home", "Balanced profile state", "No forced action"], ["Tactical Profile", "Run start", "Review"]),
  ...
  "practice-active-session-guide": defineScenario("practice-active-session-guide", "Practice", "Active session · first-use guide", "practice--active-session-guide", "Four semantic first-use explanations shown before the Sprint begins. The frozen demonstration preserves the real Sprint hierarchy while using a height-aware portrait board, the real full-width portrait Unclear prompt, fixed-shape downward portrait pointers with target clearance, and measured board-to-rail connectors that route around copy and stop short of the amber timer and Mark as unclear control in landscape.", "practice", ["No real session chessboard during guidance", "Frozen real-Sprint composition", "Height-aware portrait board", "Full-width production Unclear prompt in portrait", "Fixed-shape downward portrait arrows outside callout borders with target clearance", "Measured landscape connectors routed around copy with target clearance", "Shared session header, prompt, timer, score, and Unclear components", "SPRINT HEADER, SLOW, TIMED OUT, and UNCLEAR guidance", "Amber explains that the user is taking too long", "Current target stays bright while unrelated UI is dimmed", "Current-guide-only accessibility announcement", "Timed out is explained as a mistake and Review entry, not Unclear", "Start Sprint only after the final guide"], ["Production onboarding persistence", "Active Sprint"], "contained"),
  "practice-arrow-duel-guide": defineScenario("practice-arrow-duel-guide", "Practice", "Arrow Duel · after shared guide", "practice--arrow-duel-guide", "First-ever app entry through Arrow Duel presents the four responsive shared Active Session explanations, then a frozen real Arrow Duel composition explains that the two arrows are the user's two choices. The portrait callout follows the board without covering it; in landscape, it stays in the board's empty lower lane and uses a dot endpoint with piece-scaled clearance so it cannot read as a third move arrow.", "practice", ["Guides 1–4: responsive shared Active Session guidance", "Guide 5: frozen real Arrow Duel composition", "Height-aware portrait board and shared session components", "Single-line guide progress", "ARROW DUEL semantic callout", "The arrows show your two choices", "Portrait callout below the board", "Landscape callout in the empty board lane", "Landscape dot endpoint stops clear of the candidate piece", "Real candidate-arrow overlay", "Back returns from guide 5 to guide 4", "No real session position during guidance", "Start Arrow Duel after the final guide"], ["Production onboarding persistence", "Arrow Duel Sprint"], "contained"),
  "practice-arrow-duel-guide-only": defineScenario("practice-arrow-duel-guide-only", "Practice", "Arrow Duel · single first-use guide", "practice--arrow-duel-guide-only", "A user who already knows the shared Active Session controls sees one responsive Arrow Duel-specific guide the first time they enter the mode. The height-aware frozen real session composition identifies the two arrows as the only choices before the timer starts, with the portrait callout below the board and a landscape dot endpoint that stops clear of the candidate piece instead of appearing to be a third move arrow.", "practice", ["One guide: 1 of 1", "Height-aware frozen real Arrow Duel composition", "Shared session components", "Single-line guide progress", "ARROW DUEL semantic callout", "The arrows show your two choices", "Portrait callout below the board", "Landscape callout in the empty board lane", "Landscape dot endpoint stops clear of the candidate piece", "Real candidate-arrow overlay", "Only the two shown moves count", "No real session position during guidance", "Start Arrow Duel"], ["Production onboarding persistence", "Arrow Duel Sprint"], "contained"),
  ...
  "history-populated": defineScenario("history-populated", "History", "Populated history", "history--populated-history", "Existing populated History defaults to Needs attention, defined by Unclear or Review membership, while Slow and Timed out remain attempt labels.", "history", ["Current History layout", "Needs attention default", "Unclear or Review union", "Attempt status labels", "Applied filter summary", "Curated tags"], ["Attempt detail", "Review", "Production history query"]),
  "history-filters": defineScenario("history-filters", "History", "Filters and active filters", "history--filters-and-active-filters", "One framed advanced-filter region above the Needs attention or All selector, with Unclear and In review combined as an OR-based Attention group that keeps Needs attention selected.", "history", ["Framed advanced filters", "Attention reason OR group", "Needs attention interaction", "All sources default", "Selected theme names above", "Compact theme count below", "Collapsed 24-theme catalog", "Reset"], ["Replay puzzle"]),
