...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  "practice-home": defineScenario("practice-home", "Practice", "Home", "practice--home", "Practice home with current Run cards, progress, and the Training Focus collecting-evidence state, without duplicating the workload and entry already owned by the dedicated Review tab.", "practice", ["Bordered Run cards", "Numeric trailing Ratings", "Saved run selection", "Add Run entry", "Edit mode entry", "Progress summary", "No duplicate Review card", "Training Focus card", "Collecting-evidence state", "Tactical Profile entry"], ["Run editor", "Review tab", "History", "Settings", "Tactical Profile"]),
  ...
  "review-due": defineScenario("review-due", "Review", "Home", "review--due-queue", "The Review tab home keeps today's scheduled retries prominent while making the Filter panel and both default-open Today sections smoothly collapsible. Filter options and their persistent result summary stay together above the Review action, and quick filters update the same two Today lists instead of switching interfaces. Stable section geometry, aligned counts, a distinct retry icon, prior attempts, misses, and recency remain visible.", "review", ["Review Home", "Due metrics", "Forecast", "Filter controls above Review action", "Persistent filter summary", "Main Review action", "Animated quick filters", "Same Today sections after filtering", "Today to review expanded by default", "Completed today expanded by default", "Fixed-height section controls", "Animated height and fade", "Animated chevrons", "Aligned section counts and chevrons", "Blue retry icon", "Attempt count", "Miss count", "First-miss recency", "Last-retry recency"], ["Production review-history derivation", "Review session", "Practice"]),
  ...
  "review-filters": defineScenario("review-filters", "Review", "Filters", "review--filters", "Four action-focused quick filters use the shared short height-and-fade disclosure motion and update the existing Today to review and Completed today lists in place. Filter options and a persistent selection-and-match summary stay together above the Review action, including while the options are collapsed. The alternate Due items and Review groups interface and low-value format and pace options remain removed.", "review", ["All", "Overdue", "Missed 2+ times", "Arrow Duel", "Filter controls above Review action", "Persistent filter summary", "Animated quick filters", "Same Today sections after filtering", "Active filter control", "No Due items interface", "No Review groups"], ["Review session", "Practice"]),
