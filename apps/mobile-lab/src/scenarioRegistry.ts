...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "practice-home-edit": defineScenario("practice-home-edit", "Practice", "Edit and reorder runs", "practice--edit-and-reorder-runs", "Home edit mode with whole-card drag, a lifted picked-up state, a medium-haptic pickup preview, real-time animated insertion, arrow-button fallbacks, Run edit actions, and removal actions.", "practice", ["Whole-card drag", "Lifted picked-up card", "Medium pickup haptic preview", "Real-time insertion animation", "Arrow-button fallback", "Run edit actions", "Format-specific icons", "Removal entry"], ["Native haptic feel validation", "Run editor", "Removal confirmation", "Practice home"]),
  "practice-custom-setup": defineScenario(
    ...
    "practice",
    ["Required unique name", "Custom configuration", "Web timing range", "Linked timing defaults", "Independent warning and timeout controls", "Collapsed All themes summary", "On-demand 24-theme catalog", "Deterministic All-to-multiple selection", "All exclusivity", "Arrow Duel Opponent reply", "Five-second reply default", "Ten-second reply maximum", "Direct 600-2200 starting rating", "Add to Home"],
  ...
  "practice-run-arrow-duel-editor": defineScenario("practice-run-arrow-duel-editor", "Practice", "Arrow Duel Run editor", "practice--arrow-duel-reply-setting", "Arrow Duel's existing Edit Run screen with the default-on Opponent reply control and a direct reply-time input capped at ten seconds. The design explains the five-second default and that the Sprint and puzzle clocks pause when the reply begins, without changing the Run's Rating identity.", "practice", ["Existing Edit Run flow", "Opponent reply default on", "Off state", "Direct reply-time input", "Five-second default", "Ten-second maximum", "Sprint and puzzle clocks paused", "Shared Run Rating identity"], ["Production persistence", "Active Arrow Duel"]),
  ...
  "practice-arrow-duel-prompt": defineScenario("practice-arrow-duel-prompt", "Practice", "Arrow Duel choice and reply", "practice--arrow-duel-prompt", "The existing Arrow Duel session becomes a two-stage choice-and-reply challenge. A correct arrow plays the tempting move and opens the Run's separate configurable reply clock, which defaults to five seconds while the Sprint and puzzle clocks pause; terminal answers use Standard's brief board feedback and advance automatically without result copy, while every failure counts the whole puzzle as one mistake and sends it to Review.", "practice", ["Initial two-arrow choice", "Correct choice handoff", "Configurable opponent reply", "Sprint and puzzle clocks paused", "Main-line reply", "Any immediate mate accepted", "Standard-style green and red feedback", "Automatic next puzzle", "Brief timeout overlay", "Review consequence without result copy"], ["Production submission", "Review scheduling", "Runtime engine"], "contained"),
