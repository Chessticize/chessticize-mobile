...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "practice-run-arrow-duel-editor": defineScenario("practice-run-arrow-duel-editor", "Practice", "Arrow Duel Run editor", "practice--arrow-duel-reply-setting", "Arrow Duel's existing Edit Run screen with the default-on Opponent reply control. The design explains the separate five-second clock and separate rating identity without changing stored Run configuration.", "practice", ["Existing Edit Run flow", "Opponent reply default on", "Off state", "Independent five-second reply clock", "Separate On and Off ratings"], ["Production persistence", "Rating-key migration", "Active Arrow Duel"]),
