...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "practice-active-session-guide": defineScenario("practice-active-session-guide", "Practice", "Active session · first-use guide", "practice--active-session-guide", "First-session guide shown before the timer starts, using a fixed non-interactive example puzzle to point at the automatic Slow timer state and Timed out board overlay without revealing the real session position.", "practice", ["No real session chessboard during guidance", "Fixed example puzzle", "Timer starts after guide", "Solved, timer, and mistake metrics", "Slow timer turns amber automatically", "Timed out appears over the board and adds to Review", "Mark as unclear button explanation", "Start Sprint"], ["Production onboarding persistence", "Active Sprint"], "contained"),
