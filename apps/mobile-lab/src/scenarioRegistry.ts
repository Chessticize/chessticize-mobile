...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "practice-arrow-duel-guide": defineScenario("practice-arrow-duel-guide", "Practice", "Arrow Duel · after shared guide", "practice--arrow-duel-guide", "First-ever Arrow Duel entry keeps the four shared Active Session explanations, then uses two frozen real-session compositions: choose the stronger arrow, and after the board rewinds and the other move lands, reply for the named side. The second callout makes the proposed preparation ramp reviewable: the first-ever cue waits for Got it, cues in the next two Sprints use 1.5 seconds, and later Sprints use 1 second. The direct Exit guide control remains available without completing Arrow Duel guidance.", "practice", ["Guides 1–4: responsive shared Active Session guidance", "Guide 5: choose the stronger move", "Guide 6: Find the reply for Black", "Always-available direct guide exit without completion", "Height-aware portrait board and shared session components", "Single-line guide progress", "Two-step Arrow Duel mental model", "Other-move last-position highlight", "First cue requires Got it", "Next two Sprints 1.5-second preparation", "Later Sprints 1-second preparation", "Sprint time stays paused", "Misses and reply timeout go to Review", "Portrait callout below the board", "Landscape callout in the empty board lane", "Real candidate-arrow overlay only on the choice step", "Back traverses both Arrow Duel steps", "No real session position during guidance", "Start Arrow Duel after the final guide"], ["Production onboarding persistence", "Preparation-duration persistence", "Arrow Duel Sprint"], "contained"),
  "practice-arrow-duel-guide-only": defineScenario("practice-arrow-duel-guide-only", "Practice", "Arrow Duel · two first-use steps", "practice--arrow-duel-guide-only", "A user who already knows the shared Active Session controls sees the two Arrow Duel-specific explanations: choose the stronger move, then reply for the named side after the other move. The final callout includes the proposed first-cue Got it, next-two-Sprints 1.5-second, and later 1-second preparation ramp; direct Exit keeps this guidance eligible for the next Arrow Duel.", "practice", ["Two guides: 1 of 2 and 2 of 2", "Always-available direct guide exit without completion", "Height-aware frozen real Arrow Duel compositions", "Shared session components", "Single-line guide progress", "Choice arrows on the first step", "Explicit Black reply prompt on the second step", "Three-stage preparation ramp", "Sprint time stays paused", "Misses and reply timeout go to Review", "Portrait callout below the board", "Landscape callout in the empty board lane", "No real session position during guidance", "Start Arrow Duel"], ["Production onboarding persistence", "Preparation-duration persistence", "Arrow Duel Sprint"], "contained"),
  ...
  "practice-arrow-duel-prompt": defineScenario("practice-arrow-duel-prompt", "Practice", "Arrow Duel choice and reply", "practice--arrow-duel-prompt", "The existing Arrow Duel session previews explicit side-to-move copy across the handoff. Before the other move animates, the full-board cue asks what the named side would play after the other move and uses the smaller line only to explain when the configured reply clock begins. Once ready, the fixed-height prompt becomes Find White’s reply in the primary fixture while its supporting line simply confirms that the other move was played. Separate stories expose the proposed first-cue Got it interaction and freeze the next-two-Sprints 1.5-second and later-Sprints 1-second cue states without wiring production persistence.", "practice", ["Initial two-arrow choice", "Green confirmation before handoff", "Silent undo before the other move", "Explicit Black or White handoff question", "Smaller timing line explains input readiness", "Fixed-height side-specific reply prompt", "Original solver perspective retained", "Reply timer starts after animation", "First cue requires Got it", "Next two Sprints 1.5-second cue", "Later Sprints 1-second cue", "Ten-second default reply", "Thirty-second maximum", "Sprint and puzzle clocks paused", "Main-line reply", "Any immediate mate accepted", "Standard-style green and red feedback", "Automatic next puzzle", "Brief timeout overlay", "Review consequence without result copy"], ["Production preparation-duration persistence", "Production copy defaults", "Review scheduling", "Runtime engine"], "contained"),
  ...
  "review-arrow-duel-reply": defineScenario("review-arrow-duel-reply", "Review", "Arrow Duel reply", "review--arrow-duel-reply", "Scheduled Arrow Duel Review previews the same explicit side-to-move wording as Sprint while retaining the Run's twelve-second reply fixture. The full-board cue asks what the named side would play after the other move and the ready prompt becomes Find Black’s reply; either stage must pass for Review to pass, and timeout receives a visible Timed out handoff before Review advances.", "review", ["Existing Review session", "Stored candidate order", "Explicit Black or White handoff question", "Side-specific ready prompt", "Run-configured twelve-second countdown", "Countdown starts at input unlock", "Visible Timed out handoff", "One Review result for both stages"], ["Production copy defaults", "Production preparation-duration persistence", "Review queue", "Replay"], "contained"),
