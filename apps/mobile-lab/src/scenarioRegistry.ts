...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "practice-arrow-duel-prompt": defineScenario("practice-arrow-duel-prompt", "Practice", "Arrow Duel choice and reply", "practice--arrow-duel-prompt", "The existing Arrow Duel session previews explicit side-to-move copy across the handoff. Before the other move animates, the full-board cue asks what the replying side would play after the other move, represents that side with the shared king glyph, and tells the player to play the best reply within the configured time. Once ready, the fixed-height prompt becomes Find White’s reply in the primary fixture while its supporting line simply confirms that the other move was played. Separate stories expose the proposed first-cue Got it interaction and freeze the next-two-Sprints 1.5-second and later-Sprints 1-second cue states without wiring production persistence.", "practice", ["Initial two-arrow choice", "Green confirmation before handoff", "Silent undo before the other move", "Replying-side king glyph in the handoff question", "Action-focused reply-time instruction", "Fixed-height side-specific reply prompt", "Original solver perspective retained", "Reply timer starts after animation", "First cue requires Got it", "Next two Sprints 1.5-second cue", "Later Sprints 1-second cue", "Ten-second default reply", "Thirty-second maximum", "Sprint and puzzle clocks paused", "Main-line reply", "Any immediate mate accepted", "Standard-style green and red feedback", "Automatic next puzzle", "Brief timeout overlay", "Review consequence without result copy"], ["Production preparation-duration persistence", "Production copy defaults", "Review scheduling", "Runtime engine"], "contained"),
