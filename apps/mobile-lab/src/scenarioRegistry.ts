...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "practice-active-session-guide": defineScenario("practice-active-session-guide", "Practice", "Active session · first-use guide", "practice--active-session-guide", "Four-step spotlight tour shown before the timer starts. A fixed production chessboard demonstrates the real session metrics, automatic Slow timer state, Timed out board overlay, and Mark as unclear control without revealing the real session position.", "practice", ["No real session chessboard during guidance", "Frozen production chessboard", "Four-step Back and Next tour", "Spotlight scrim and anchored explanations", "Solved, Sprint, and Mistakes metrics", "Slow timer turns amber automatically", "Timed out appears over the board and adds to Review", "Mark as unclear button explanation", "Start Sprint only on the final step"], ["Production onboarding persistence", "Active Sprint"], "contained"),
