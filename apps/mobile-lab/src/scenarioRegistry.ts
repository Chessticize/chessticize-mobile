...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "review-due": defineScenario("review-due", "Review", "Home", "review--due-queue", "The Review tab home keeps today's scheduled retries prominent while making the Filter panel and both default-open Today sections smoothly collapsible. Quick filters update those same two lists instead of switching to a second Review interface, while stable section geometry, aligned counts, a distinct retry icon, prior attempts, misses, and recency remain visible.", "review", ["Review Home", "Due metrics", "Forecast", "Main Review action", "Animated quick filters", "Same Today sections after filtering", "Today to review expanded by default", "Completed today expanded by default", "Fixed-height section controls", "Animated height and fade", "Animated chevrons", "Aligned section counts and chevrons", "Blue retry icon", "Attempt count", "Miss count", "First-miss recency", "Last-retry recency"], ["Production review-history derivation", "Review session", "Practice"]),
  ...
  "review-filters": defineScenario("review-filters", "Review", "Filters", "review--filters", "Four action-focused quick filters use the shared short height-and-fade disclosure motion and update the existing Today to review and Completed today lists in place. The design removes the alternate Due items and Review groups interface, low-value format and pace options, and duplicate summary chips.", "review", ["All", "Overdue", "Missed 2+ times", "Arrow Duel", "Animated quick filters", "Same Today sections after filtering", "Active filter control", "No Due items interface", "No Review groups", "No duplicate summary chips"], ["Review session", "Practice"]),
