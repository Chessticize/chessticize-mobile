...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "practice-run-arrow-duel-editor": defineScenario("practice-run-arrow-duel-editor", "Practice", "Arrow Duel Run editor", "practice--arrow-duel-reply-setting", "Arrow Duel's existing Edit Run screen with the default-on Opponent reply control and a direct reply-time input capped at ten seconds. The design explains the five-second default, that the Sprint and puzzle clocks pause when the reply begins, and that On and Off keep separate ratings without changing stored Run configuration.", "practice", ["Existing Edit Run flow", "Opponent reply default on", "Off state", "Direct reply-time input", "Five-second default", "Ten-second maximum", "Sprint and puzzle clocks paused", "Separate On and Off ratings"], ["Production persistence", "Rating-key migration", "Active Arrow Duel"]),
  ...
  "practice-arrow-duel-prompt": defineScenario("practice-arrow-duel-prompt", "Practice", "Arrow Duel choice and reply", "practice--arrow-duel-prompt", "The existing Arrow Duel session becomes a two-stage choice-and-reply challenge. A correct arrow plays the tempting move and opens the Run's separate configurable reply clock, which defaults to five seconds while the Sprint and puzzle clocks pause; a wrong choice, wrong reply, or timeout counts the whole puzzle as one mistake and sends it to Review.", "practice", ["Initial two-arrow choice", "Correct choice handoff", "Configurable opponent reply", "Sprint and puzzle clocks paused", "Main-line reply", "Any immediate mate accepted", "Whole-puzzle correct state", "Wrong choice", "Wrong reply", "Reply timeout", "Review consequence"], ["Production submission", "Review scheduling", "Rating identity", "Runtime engine"], "contained"),
  "practice-arrow-duel-mate-in-one": defineScenario("practice-arrow-duel-mate-in-one", "Practice", "Arrow Duel reply · alternate mate", "practice--arrow-duel-reply-alternate-mate", "A sampled Core Pack position where the stored reply and another legal move both checkmate immediately. The alternate mate is accepted without consulting an engine.", "practice", ["Correct initial choice", "Tempting move handoff", "Stored main-line reply", "Alternate legal mate in one", "Accepted solved state", "No runtime engine"], ["Production submission", "Review scheduling", "Rating identity"], "contained"),
