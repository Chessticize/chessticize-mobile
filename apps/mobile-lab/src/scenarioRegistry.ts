...
const scenarioDefinitions: Record<LabScenarioId, LabScenarioMetadata> = {
  ...
  "practice-run-arrow-duel-editor": defineScenario("practice-run-arrow-duel-editor", "Practice", "Arrow Duel Run editor", "practice--arrow-duel-reply-setting", "Arrow Duel's existing Edit Run screen keeps the per-Run Opponent reply control and direct reply-time input only while the global setting is on. Its copy now says that the override controls this Run only; a separate global-off story proves the section disappears without changing the rest of Edit Run or the Run's Rating identity.", "practice", ["Existing Edit Run flow", "Visible only while global on", "Individual Run scope", "Per-Run off state", "Direct reply-time input", "Ten-second default", "Thirty-second maximum", "Sprint and puzzle clocks paused", "Global-off absence", "Saved Run intent preserved by later wiring", "Shared Run Rating identity"], ["Production persistence", "Global setting wiring", "Active Arrow Duel"]),
  ...
  "settings-ios-sync": defineScenario(
    ...
    "iOS sync",
    "settings--ios-sync",
    "The stable iOS Settings clone adds the global Arrow Duel Opponent reply challenge control alongside iCloud Sync, Notifications, Move Feedback, and Guidance. Its on and off copy explains the classic one-choice fallback, per-Run Edit Run override, and saved-intent preservation.",
    "settings",
    ["iCloud Sync", "Global Arrow Duel opponent-reply control", "Optional explanation", "Per-Run Edit Run handoff", "One-choice fallback", "Saved Run intent preserved", "Notifications", "Sound and haptic toggles", "Move and capture audio previews", "Guidance reset", "About"],
  ...
  "settings-android-backup": defineScenario("settings-android-backup", "Settings", "Android backup", "settings--android-backup", "Android managed-backup Settings keeps iCloud controls omitted and its local SQLite diagnostics export under Help & Feedback while sharing the same global Arrow Duel Opponent reply challenge control and explanation as iOS.", "settings", ["Android Progress Backup", "Global Arrow Duel opponent-reply control", "Optional explanation", "Per-Run Edit Run handoff", "One-choice fallback", "Local SQLite support bundle", "Diagnostic manifest", "Android share handoff", "Notifications", "About"], ["Native SQLite snapshot and Android share options", "Global setting wiring"]),
