...
5. Add `isNew: true`, the owning `issueNumber`, and a one-line `changeNote` to each new or materially changed registry entry. This adds the Storybook `new` tag and the What's New card.
...
8. Keep the marker while the linked issue remains open. Remove it only after the issue closes, while retaining the scenario as living UI documentation.
