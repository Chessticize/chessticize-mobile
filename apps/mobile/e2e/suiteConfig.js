...
const IOS_LANDSCAPE_LAYOUT_TEST_MATCH = MARKETING_ASSETS_TEST_MATCH;
...
const RESOURCE_SOAK_TEST_MATCH = ['<rootDir>/e2e/resource-soak.e2e.js'];
...
function resolveDetoxTestMatch(environment = process.env) {
  ...
  const verifyIosLandscapeLayout = environment.CHESSTICIZE_VERIFY_IOS_LANDSCAPE_LAYOUT === '1';
  ...
  if ([captureStoreAssets, captureMarketingAssets, verifyIosLandscapeLayout, captureAdaptiveLayout, runResourceSoak, Boolean(activeSuite)].filter(Boolean).length > 1) {
  ...
  if (verifyIosLandscapeLayout) {
    return IOS_LANDSCAPE_LAYOUT_TEST_MATCH;
  }
  ...
  if (runResourceSoak) {
    return RESOURCE_SOAK_TEST_MATCH;
  }
